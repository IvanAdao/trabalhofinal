...
-- Tabela: Role (papéis de acesso)
CREATE TABLE Role (
    id_role INT PRIMARY KEY AUTO_INCREMENT,
    nome VARCHAR(50) UNIQUE -- Ex: 'RH', 'AUDITOR', 'ADMIN'
);

-- Tabela: Utilizador_Role (ligação N:N)
CREATE TABLE Utilizador_Role (
    id_utilizador INT,
    id_role INT,
    PRIMARY KEY (id_utilizador, id_role),
    FOREIGN KEY (id_utilizador) REFERENCES Utilizador(id_utilizador),
    FOREIGN KEY (id_role) REFERENCES Role(id_role)
);

-- Tabela: Servidor_Publico (com 'nuit' em vez de 'numero_bi')
CREATE TABLE Servidor_Publico (
    id_servidor INT PRIMARY KEY AUTO_INCREMENT,
    nome_completo VARCHAR(255),
    nuit VARCHAR(100) UNIQUE,
    nif VARCHAR(100),
    cargo VARCHAR(100),
    orgao VARCHAR(100),
